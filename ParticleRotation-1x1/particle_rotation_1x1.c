...
void draw(void)
{
	...
	FILE *mtixB=fopen("matrix_B.dat","wt");
	FILE *mtixX=fopen("matrix_X.dat","wt");

	float Uinf, Vinf;
	float cycx[N]={0,}, cycy[N]={0,}, xmid[N]={0,}, ymid[N]={0,}, deltaSn[N]={0,}, angle;
	float x[N], y[N], tx[N], ty[N], nx[N], ny[N], cosbetam[N]={0,}, sinbetam[N]={0,};	
		
	float K_Sm_Sn[N][N]={0,}, inv_Kmn[N][N]={0,}, Surface_gam[N]={0,}, rhsM[N]={0,};
	float rhsM_Winf[N]={0,}, Surface_gam_Winf[N]={0,};	//for Martensen

	float rmj[N]={0,}, Umj[N]={0,}, Vmj[N]={0,};
	float Umn[N]={0,}, Vmn[N]={0,};

	float delta_GAM_A[N], deltaT, epsl_length, ceta;			//two shed points
	...
	float sigma_rhsm_for_mj_sec[N][N]={0,}, sigma_rhsm_mj_sec[N]={0,}, sigma_rhsm_sec=0.0;					//for K function's second sigma item
...
//*************************************||        Martensen analysis Winf only       ||*************************************
	...
	for(i=0;i<M-1;i++)																//Martensen analysis
	{
		for(j=0;j<M-1;j++)
		{
			K_Sm_Sn[i][j] = cacu_K_Sm_Sn(deltaSn[j], xmid[i],ymid[i],xmid[j],ymid[j],cosbetam[i],sinbetam[i]) + deltaSn[j];	//(XM, YM, XN, YN, COS, SIN)
			fprintf(mtixA, "%8.4f  ", K_Sm_Sn[i][j]);
		}
		fprintf(mtixA, "\n", K_Sm_Sn[i][j]);
...
//*************************************||            Shed two free vortex           ||*************************************
	...
	for(time = 0; time < iter; time++)
	{
		...
		if(time != 0)
		{
			for(i=0;i<1;i++)
			{
				delta_GAM_A[time+i] = cacu_deltaGAM(Surface_gam[3], deltaT);
				printf("%0.4f\n", delta_GAM_A[time+i]);
			}
		}
//*************************************||           Caculate the Umj & Vmj          ||*************************************
		...
		for(i=0;i<VortexNum;i++)
		{
			for(j=0;j<M-1;j++)
			{
				...
				sigma_rhsm_for_mj_sec[i][j] = delta_GAM_A[i] * (1.0 + Umj[j] * cosbetam[j] + Vmj[j] * sinbetam[j]);
			}
		}
	
		for(i=0;i<M-1;i++)
		{
			sigma_rhsm_mj_sec[i] = 0.0;
			for(j=0;j<VortexNum;j++)
			{
				sigma_rhsm_mj_sec[i] += sigma_rhsm_for_mj_sec[j][i];
			}
		}
		sigma_rhsm_sec = 0.0;
		for(i=0;i<M-1;i++)
		{
			sigma_rhsm_sec += sigma_rhsm_mj_sec[i];
		}

		for(i=0;i<M-1;i++)
		{
			rhsM[i] = rhsM_Winf[i] - sigma_rhsm_mj_sec[i];
		//	rhsM[i] = rhsM_Winf[i] - sigma_rhsm_sec;
		}
		...
		if(VortexNum>1)
		{
			for(i=0;i<VortexNum;i++)									//init two item
			{
				...
				for(j=0;j<VortexNum;j++)
				{
					if(i!=j)
					{
						udm_sig_one[i] += delta_GAM_A[i] * cacu_Umn(point_A_x[i], point_A_y[i], point_A_x[j], point_A_y[j]);
						vdm_sig_one[i] += delta_GAM_A[i] * cacu_Vmn(point_A_x[i], point_A_y[i], point_A_x[j], point_A_y[j]);
					}	
				}
			}
		}
...
//*************************************||             Vortex convection             ||*************************************
		...
		for(i=0;i<VortexNum;i++)									//init two item
		{
			point_A_x_next[i] = point_A_x[i] + (udm[i]) * deltaT;
			point_A_y_next[i] = point_A_y[i] + (vdm[i]) * deltaT;
		}
