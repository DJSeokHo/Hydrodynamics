...
#define iter 50			//number of iterations
...
void draw(void)
{
	...
	FILE *surfacegam=fopen("surfacegam.dat","wt");
	...
	float randomQ[N], ri[N];
	float del_cetai[N], del_ri[N];
...
//*************************************||        Martensen analysis Winf only       ||*************************************
	...
	for(i=0;i<M-1;i++)																//Martensen analysis
	{
		...
		//	fprintf(mix_A, "%8.4f ", K_Sm_Sn_Winf[i][j]);
		}
	//	fprintf(mix_A, "\n");
...
//*************************************||          Matrix inversion Winf only       ||*************************************
	...
	temp_surface_GAM[1] = Surface_gam_Winf[5];
...
//	printf("%0.4f  %0.4f\n", delta_GAM[0], delta_GAM[1]);
	...
	shed_x[1] = epsl_length*(float)cos(ang_to_rad(ceta))+xmid[6];
	shed_y[1] = ymid[6]-epsl_length*(float)sin(ang_to_rad(ceta));
	...
	//	printf("\n");
		...
		for(i=0;i<M-1;i++)																//Martensen analysis
		{
			...
			//	K_Sm_Sn[i][j] = cacu_K_Sm_Sn(Coeff[j], xmid[i],ymid[i],xmid[j],ymid[j],cosbetam[i],sinbetam[i]);	//(XM, YM, XN, YN, COS, SIN)
				...
				fprintf(mix_A, "%0.4f ", K_Sm_Sn[i][j]);
			}
			fprintf(mix_A, "\n");
		}
		fprintf(mix_A, "\n");
		...
		temp_surface_GAM[1] = Surface_gam[5];
...
//*************************************||     Cacu the convection of Umn & Vmn      ||*************************************
		...
		for(i=0;i<VortexNum;i++)
		{
			...
			fprintf(test, "%d  %0.4f  %0.4f\n", i+1, udm_sig_one[i], vdm_sig_one[i]);
		}
		fprintf(test, "\n");
...
//*************************************||              Vortex diffstion             ||*************************************
		...
		for(i=0;i<VortexNum;i++)
		{
			...
			del_ri[i] = cacu_ri(ri[i], 0.001);
			
	//		printf("%0.4f  %0.4f\n", del_cetai[i], del_ri[i]);

	//		point_x_next[i] = point_x_next[i] + del_ri[i] * (float)cos(del_cetai[i]);
	//		point_y_next[i] = point_y_next[i] + del_ri[i] * (float)sin(del_cetai[i]);
		}
