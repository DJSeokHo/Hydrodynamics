...
float cacu_deltaGAM(float, float);	//set shed points
...
void draw(void)
{
	int i, j, k, time, VortexNum, VortexIndex, m;		//for Loop
	FILE *circle=fopen("circle.dat","wt");				//for geometry
	FILE *pivotalP=fopen("pivotalP.dat","wt");			//for geometry
	FILE *shed=fopen("shed.dat","wt");			//for shed vortices
	FILE *driftPathALL=fopen("driftPathALL.dat","wt");	//for driftPath A
	...
	float K_Sm_Sn_Winf[N][N]={0,}, inv_Kmn_Winf[N][N]={0,}, Surface_gam_Winf[N]={0,}, rhsM_Winf[N]={0,};	
																						//for Martensen
	float K_Sm_Sn[N][N]={0,}, inv_Kmn[N][N]={0,}, Surface_gam[N]={0,}, rhsM[N]={0,};	//for Martensen
	float Surface_gam_addA[N]={0,};														//for Martensen									

	float Surface_gamA[N]={0,};

	float rmj[N]={0,}, Umj[N]={0,}, Vmj[N]={0,};								//for Umj&Vmj convection
	float Umn[N]={0,}, Vmn[N]={0,};												//for Umn&Vmn convection

	float delta_GAM[N], deltaT;														//shed points

	float shed_x[N], shed_y[N];													//shed points

	float point_x[N]={0,}, point_y[N]={0,};										//shed points
	
	float point_x_next[N][N]={0,}, point_y_next[N][N]={0,};						//for drift path

	float sigma_rhsm_for_mj[N][N]={0,}, sigma_rhsm_mj[N]={0,};					//for K function's second sigma item

	float udmALL[N][N]={0,}, vdmALL[N][N]={0,};
	float udm_sig_one[N][N]={0,}, udm_sig_two[N][N]={0,}, vdm_sig_one[N][N]={0,}, vdm_sig_two[N][N]={0,};

	float outputstreamL_x[N][N]={0,}, outputstreamL_y[N][N]={0,};
...
//*************************************||        Martensen analysis Winf only       ||*************************************
	...
	for(i=0;i<M-1;i++)																//Martensen analysis
	{
		for(j=0;j<M-1;j++)
		{
			...
			fprintf(mixA, "%8.5f  ", K_Sm_Sn_Winf[i][j]);
		}
		fprintf(mixA, "\n");
...
//*************************************||          Matrix inversion Winf only       ||*************************************
	...
	deltaT = 0.05;		//time steps
	

//*************************************||                  Start Loop               ||*************************************

	for(time = 0; time < iter; time++)					//Loop of time steps
	{
		VortexNum = (time + 1) * (M - 1);				//Vortex number follow time steps	

//*************************************||            Shed M free vortices           ||*************************************
		...
		for(i=0;i<VortexNum;i++)
		{
			...
			delta_GAM[i] = cacu_deltaGAM(Surface_gam_Winf[i], deltaSn[i]);		//strength of vortices
		//	printf("%d  %2.4f\n", i+1, delta_GAM_ALL[i]);
		}

		for(i=0;i<VortexNum;i++)			//save shed vortices in to point[] for calculation
		{
			point_x[i] = shed_x[i];
			point_y[i] = shed_y[i];
		}
		
//*************************************||           Calculate the Umj & Vmj          ||*************************************
	
		for(i=0;i<VortexNum;i++)			//i: M free vortices
		{
			for(j=0;j<M-1;j++)				//j: M surfaces
			{
				rmj[j] = cacu_rm(xmid[j], point_x[i], ymid[j], point_y[i]);
				Umj[j] = cacu_Um(ymid[j], point_y[i], rmj[j]);
				Vmj[j] = cacu_Vm(xmid[j], point_x[i], rmj[j]);
				sigma_rhsm_for_mj[i][j] = cacu_rhsm(Umj[j], cosbetam[j], Vmj[j], sinbetam[j], delta_GAM[i]);
			}
		}
...
float cacu_deltaGAM(float gam, float deltaS)
{
	float temp_deltaGAM_, temp_gam_, temp_deltaS_;
	temp_gam_=gam; temp_deltaS_=deltaS;
	temp_deltaGAM_=2 * temp_gam_ * temp_deltaS_;
