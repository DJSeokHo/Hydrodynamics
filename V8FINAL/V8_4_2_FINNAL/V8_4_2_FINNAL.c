...
#define GAM 1.0				//GAM
#define iter 1				//number of iteration
...
void draw(void)
{
	int i, j, k, p, VortexNum, time, nearest, nsubs[N][N]={0,};			//for Loop
	...
	float K_Sm_Sn[N][N]={0,}, inv_Kmn[N][N]={0,}, Surface_gam[N][N]={0,}, rhsM[N][N]={0,};	//for Martensen

	float xja[N], yja[N], xjb[N], yjb[N], xjd[N], yjd[N];
	float rmj[N][N]={0,}, Umj[N][N]={0,}, Vmj[N][N]={0,};

	float tempX[N][N]={0,}, tempY[N][N]={0,}, tempsubs[N][N]={0,}, xn[N][N]={0,}, yn[N][N]={0,};

	float rnj[N][N]={0,};
	float sigmaU[N][N]={0,}, sigmaV[N][N]={0,};
	float NewUmj[N][N]={0,}, NewVmj[N][N]={0,};
	
	float temp_rmj[N][N]={0,};

	float ud[N], vd[N], ub[N], vb[N], ub_sig[N], vb_sig[N], ud_sig[N], vd_sig[N], deltaT;
...
//*************************************||           Caculate the Umj & Vmj          ||*************************************
	for(time=0;time<iter;time++)
	{
		...
		for(p=0;p<VortexNum;p++)
		{
			for(i=0;i<M-1;i++)
			{
				rmj[p][i] = cacu_rm(xmid[i], xja[p], ymid[i], yja[p]);
				Umj[p][i] = cacu_Um(ymid[i], yja[p], rmj[p][i]);
				Vmj[p][i] = cacu_Vm(xmid[i], xja[p], rmj[p][i]);
			}	

//*************************************||       Caculate sub-elements Umj & Vmj     ||*************************************

			for(i=0;i<M-1;i++)					//each surface
			{
				nsubs[p][i] = cacu_nsubs(rmj[p][i], deltaSn[i]);
				tempsubs[p][i] = (1/(float)(nsubs[p][i]));	

				sigmaU[p][i] = 0.0; sigmaV[p][i] = 0.0;
		
				for(j=0; j<nsubs[p][i]; j++)	//each sub-element of each surface
				{	
					if(nsubs[p][i]==1)
					{
						tempX[p][j] = cycx[i];
						tempY[p][j] = cycy[i];
						tempX[p][j+1] = cycx[i+1];
						tempY[p][j+1] = cycy[i+1];
					}

					else if( (nsubs[p][i]!=1) && (j!=(nsubs[p][i]-1)) )
					{
						tempX[p][0] = cycx[i];
						tempY[p][0] = cycy[i];		
						tempX[p][j+1] = init_tempXY(cycx[i+1], cycx[i], tempsubs[p][i]);
						tempY[p][j+1] = init_tempXY(cycy[i+1], cycy[i], tempsubs[p][i]);
						tempsubs[p][i] += (1/(float)(nsubs[p][i]));
						tempX[p][j+2] = cycx[i+1];
						tempY[p][j+2] = cycy[i+1];
					}
				...
				for(j=0; j<nsubs[p][i]; j++)
				{
					xn[p][j] = init_cp(tempX[p][j+1], tempX[p][j]);
					yn[p][j] = init_cp(tempY[p][j+1], tempY[p][j]);
					rnj[p][j] = cacu_rn(xn[p][j], xja[p], yn[p][j], yja[p]);
					sigmaU[p][i] += ((yn[p][j]-yja[p])/(float)pow(rnj[p][j],2));		
					sigmaV[p][i] += ((xn[p][j]-xja[p])/(float)pow(rnj[p][j],2));
					fprintf(sub, "%0.5f  %0.5f\n", xn[p][j], yn[p][j] );
				}
				NewUmj[p][i] = cacu_New_Um(sigmaU[p][i], nsubs[p][i]);
				NewVmj[p][i] = cacu_New_Vm(sigmaV[p][i], nsubs[p][i]);
				printf("%d  %0.4f, %0.4f\n", i+1, NewUmj[p][i], NewVmj[p][i]);		
			}
	...
	//*************************************||        Martensen analysis Winf only       ||*************************************
		...
		for(p=0;p<VortexNum;p++)
		{
			for(i=0;i<M-1;i++)
			{
				rhsM[p][i] = cacu_rhsm(NewUmj[p][i], cosbetam[i], NewVmj[p][i], sinbetam[i]);	//(U, cos, V, sin)
			}
		}
	//*************************************||                Matrix inversion           ||*************************************
		...
		for(p=0;p<VortexNum;p++)
		{
			...
			for(i=0;i<M-1;i++)
			{
				...
				for(j=0;j<M-1;j++)
				{
					Surface_gam[p][i]+=inv_Kmn[i][j]*rhsM[p][j];				//Get surface GAM
				}
			//	printf("%0.4f\n", Surface_gam[p][i]);
			}
		}
	...
	//*************************************||         Star Converction drift path       ||*************************************
		...
		for(p=0;p<VortexNum;p++)
		{
			ub_sig[p] = 0.0; vb_sig[p] = 0.0;					//init ud & vd
			for(i=0;i<M-1;i++)
			{
				ub_sig[p] += Surface_gam[p][i] * deltaSn[i] * NewUmj[p][i];
				vb_sig[p] += Surface_gam[p][i] * deltaSn[i] * NewVmj[p][i];
			}
		
			ub[p] = (-1) * ub_sig[i];
			vb[p] = (-1) * vb_sig[i];
		
			xjb[p] = xja[p] + ub[p] * deltaT;
			yjb[p] = yja[p] + vb[p] * deltaT;

			fprintf(driftPath, "%0.4f %0.4f\n", xjb[p], yjb[p]);

			xja[p] = xjb[p]; yja[p] = yjb[p];
		}
