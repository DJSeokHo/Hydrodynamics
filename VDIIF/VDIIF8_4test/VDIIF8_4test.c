...
#define GAM 1.0								//is three point's GAM value different?
#define GAMJ 0.05
#define GAMI 1.0
#define GAM0 1.0
#define a 0.5
...
float cacu_um(float, float, float, float);             //Get um
float cacu_vm(float, float, float, float);             //Get vm
...
//**************************************** from a to b ****************************************
	for(time=0; time<100; time++)
	{
		...
		tempx = init_x(tempAngleCos, a);		//Calculate (x,y) of circle
		tempy = init_y(tempAngleSin, a);		//Calculate (x,y) of circle
	...
	//		rhsm += rhsN[i] * deltaSn[i];
			...
			um[i] = cacu_um(Um0[i], Umi[i], Umj[i], GAMJ, GAMI, GAM0);
			vm[i] = cacu_vm(Vm0[i], Vmi[i], Vmj[i], GAMJ, GAMI, GAM0);
...
float cacu_um(float M0, float Mi, float Mj, float Gam)
{
	float temp_um_, temp_m0_, temp_mi_, temp_mj_, temp_gam_;
	temp_m0_=M0; temp_mi_=Mi; temp_mj_=Mj; temp_gam_=Gam;

	temp_um_=(temp_m0_ - temp_mi_  + temp_mj_) * temp_gam_;
	return temp_um_;
	temp_um_=0; temp_m0_=0; temp_mi_=0; temp_mj_=0; temp_gam;
}

float cacu_vm(float M0, float Mi, float Mj, float Gam)
{
	float temp_vm_, temp_m0_, temp_mi_, temp_mj_, temp_gam_;
	temp_m0_=M0; temp_mi_=Mi; temp_mj_=Mj; temp_gam_=Gam;

	temp_vm_=(temp_m0_ - temp_mi_  + temp_mj_) * temp_gam_;
	return temp_vm_;
	temp_vm_=0; temp_m0_=0; temp_mi_=0; temp_mj_=0; temp_gam_=0;
}
