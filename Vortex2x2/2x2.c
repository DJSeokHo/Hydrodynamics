...
float cacu_U_32(float, float, float, float, float);
float cacu_V_32(float, float, float, float, float);
float cacu_u_32(float, float);
float cacu_v_32(float, float);
...
void draw(void)
{
	int i, j, k, time, VortexNum;			//for Loop
	...
	float Surface_gam_Winf[N]={0,}, rhsM_Winf[N]={0,};	//for Martensen
	...
	float delta_GAM[N], deltaT, epsl_length, ceta;			//all shed points strength and delta T
...
//*************************************||        Martensen analysis Winf only       ||*************************************
	...
	for(i=0;i<M-1;i++)																//Martensen analysis
	{
		for(j=0;j<M-1;j++)
		{
			K_Sm_Sn[i][j] = cacu_K_Sm_Sn(deltaSn[j], xmid[i],ymid[i],xmid[j],ymid[j],cosbetam[i],sinbetam[i]);
			fprintf(mix_A_Winf, "%0.6f ", K_Sm_Sn[i][j]);
		}
		fprintf(mix_A_Winf, "\n");
...
//*************************************||          Matrix inversion Winf only       ||*************************************
	...
	for(i=0;i<M-1;i++)
	{
		...
		for(j=0;j<M-1;j++)
		{
			Surface_gam_Winf[i]+=inv_Kmn[i][j]*rhsM_Winf[j];				//Get surface GAM
		}
...
*/
	...
	ceta = 45;	
	shed_x[0]=epsl_length*(float)cos(ang_to_rad(ceta))+xmid[2];
	shed_y[0]=epsl_length*(float)sin(ang_to_rad(ceta))+ymid[2];
	shed_x[1]=epsl_length*(float)cos(ang_to_rad(ceta))+xmid[7];
	shed_y[1]=ymid[7]-epsl_length*(float)sin(ang_to_rad(ceta));

	fprintf(shed, "%0.4f %0.4f\n", shed_x[2], shed_y[2]);
	fprintf(shed, "%0.4f %0.4f\n", shed_x[7], shed_y[7]);
//	outputstreamL_x[iter+1] = shed_x[2];
//	outputstreamL_y[iter+1] = shed_y[2];
	
//	outputstreamL_x[iter+2] = shed_x[7];
//	outputstreamL_y[iter+2] = shed_y[7];

	delta_GAM[0] = cacu_deltaGAM(Surface_gam_Winf[3], deltaT);
	delta_GAM[1] = (-1)*cacu_deltaGAM(Surface_gam_Winf[3], deltaT);
...
//	printf("%0.4f\n", delta_GAM[0]);
	for(time = 0; time < iter; time++)
	{
		...
		for(i=0;i<2;i++)
		{
			point_x[time*2+i] = shed_x[i];
			point_y[time*2+i] = shed_y[i];

	//		printf("%d %0.4f  %0.4f\n", time*2+i, point_x[time*2+i], point_y[time*2+i]);
		}
		...
		if(time != 0)
		{
			for(i=0;i<2;i++)
			{
				delta_GAM[time*2+0] = cacu_deltaGAM(Surface_gam_Winf[3], deltaT);
				delta_GAM[time*2+1] = (-1) * cacu_deltaGAM(Surface_gam_Winf[3], deltaT);
			//	printf("%d %0.4f  %0.4f\n", i, delta_GAM[time*2+0], delta_GAM[time*2+1]);
			}
		}
		...
		for(i=0;i<VortexNum;i++)
		{
			...
			for(j=0;j<VortexNum;j++)
			{
				...
				//	vdm_sig_one[i] += delta_GAM[i] * cacu_Vmn(point_x[i], point_y[i], point_x[j], point_y[j]);
					Umn[j] = cacu_U_32(point_x[i], point_y[i], point_x[j], point_y[j], delta_GAM[i]);
					Vmn[j] = cacu_V_32(point_x[i], point_y[i], point_x[j], point_y[j], delta_GAM[i]);
					udm_sig_one[i] += cacu_u_32(Umn[j], delta_GAM[j]);
					vdm_sig_one[i] += cacu_v_32(Vmn[j], delta_GAM[j]);
				}
			}
		}
...
//*************************************||   prepare velocity for Vortex convection  ||*************************************
		...
		//	vdm[i] = vdm_sig_one[i] + vdm_sig_two[i] + Vinf;
			udm[i] = udm_sig_one[i] + Uinf;
			vdm[i] = vdm_sig_one[i] + Vinf;
...
float cacu_U_32(float Xm, float Ym, float Xn, float Yn, float DGAM)
{
	float temp_U, tempXm, tempYm, tempXn, tempYn, temp_dgam;
	tempXm=Xm; tempYm=Ym; tempXn=Xn; tempYn=Yn; temp_dgam=DGAM;
	temp_U = (temp_dgam/(2*TL))*( (float)sinh(((2*PI)/TL)*(tempYm-tempYn))/((float)cosh(((2*PI)/TL)*(tempYm-tempYn))-(float)cos(((2*PI)/TL)*(tempXm-tempXn))));
	return temp_U;
}

float cacu_V_32(float Xm, float Ym, float Xn, float Yn, float DGAM)
{
	float temp_V, tempXm, tempYm, tempXn, tempYn, temp_dgam;
	tempXm=Xm; tempYm=Ym; tempXn=Xn; tempYn=Yn; temp_dgam=DGAM;
	temp_V = (-1)*(temp_dgam/(2*TL))*( (float)sin(((2*PI)/TL)*(tempXm-tempXn))/((float)cosh(((2*PI)/TL)*(tempYm-tempYn))-(float)cos(((2*PI)/TL)*(tempXm-tempXn))));
	return temp_V;
}

float cacu_u_32(float U, float DGAM)
{
...
float cacu_v_32(float V, float DGAM)
{
