...
//*************************************|| Input profile and other data preparation  ||*************************************
	...
	{																				//   init   //										// sinbetam //
		cosbetam[i] = tx[i];
		sinbetam[i] = ty[i];
	//	printf("cosbeta = %0.4f , sinbeta = %0.4f\n", cosbetam[i], sinbetam[i]);
	}	
...
//*************************************||        Martensen analysis Winf only       ||*************************************
	...
	for(i=0;i<M-1;i++)																//Martensen analysis
	{
		for(j=0;j<M-1;j++)
		{
			K_Sm_Sn_Winf[i][j] = cacu_K_Sm_Sn_Winf(Coeff[j], xmid[i],ymid[i],xmid[j],ymid[j],cosbetam[j],sinbetam[j]) + deltaSn[j];  // ==>deltaSn ¥ı«“ ∞Õ  
			fprintf(mix_A_Winf, "%0.6f ", K_Sm_Sn_Winf[i][j]);
		}
		fprintf(mix_A_Winf, "\n");
//		rhsM_Winf[i] = cacu_rhsm_Winf(Uinf, cosbetam[i], Vinf, sinbetam[i]) ;	//(U, cos, V, sin)
		rhsM_Winf[i] = -Uinf*cosbetam[i] - Vinf*sinbetam[i] ;
//				printf( "%0.6f\n", rhsM_Winf[i]);
...
//*************************************||           Caculate the Umj & Vmj          ||*************************************
		...
		for(i=0;i<M-1;i++)
		{
			...
			for(j=0;j<VortexNum;j++)
			{
				...
				sigma_rhsm_for_mj[j] = (-1) * delta_GAM[j] * (1.0 + Umj[j] * cosbetam[i] + Vmj[j] * sinbetam[i]);
...
//*************************************||             Vortex convection             ||*************************************
		...
		for(i=0;i<VortexNum;i++)									
		{
			point_x_next[i] = point_x[i] + (udm[i] + Uinf) * deltaT;
			point_y_next[i] = point_y[i] + (vdm[i] + Vinf) * deltaT;
		}
