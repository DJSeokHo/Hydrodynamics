...
#define iter 2			//number of iterations
...
void draw(void)
{
	int i, j, k, time, VortexNum, VortexIndex, eliminateNum=0;			//for Loop
	...
	float point_x_e[N]={0,}, point_y_e[N]={0,};
...
//*************************************||            Shed all free vortex           ||*************************************
	...
	for(time = 0; time < iter; time++)
	{
		...
		VortexNum = (time + 1) * (M-1) - eliminateNum;			 //Vortex number follow time steps 
		eliminateNum = 0;										 //Two vortices crate for per step
		for(i=0;i<M-1;i++)
		{
...
//*************************************||             Vortex convection             ||*************************************
		...
		for(i=0;i<VortexNum;i++)									
		{
			...
			printf("%d %0.4f  %0.4f\n", i+1, point_x_next[i], point_y_next[i]);
		}
...
//*************************************||              Vortex Eliminate             ||*************************************
		...
		for(i=0;i<VortexNum;i++)									
		{
			eliminateR = (float)sqrt((float)pow((point_x_next[i] - 0.0),2) + (float)pow((point_y_next[i] - 0.0),2));
			if(eliminateR<=a)
			{
				eliminateNum++;
				point_x_e[i] = point_x_next[i+eliminateNum];
				point_y_e[i] = point_y_next[i+eliminateNum];
			}
			else
			{
				point_x_e[i] = point_x_next[i];
				point_y_e[i] = point_y_next[i];
			}

			printf("%d %0.4f  %0.4f\n", i+1, point_x_e[i], point_y_e[i]);
		}
...
//*************************************||             Output streamline             ||*************************************
		...
		for(i=0;i<VortexNum;i++)									
		{
			point_x[i] = point_x_e[i];
			point_y[i] = point_y_e[i];
		}
		...
		if(time == iter - 1)
		{
			for(i=0;i<VortexNum-eliminateNum;i++)
			{	
