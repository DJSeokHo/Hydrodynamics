...
float cacu_K_Sm_Sn(float, float, float, float, float, float, float);	//cacu K(Sm, Sn), (COE, XM, YM, XN, YN, COS, SIN)
float cacu_rhsm(float, float, float, float);					//cacu rhsm
float cacu_slope(float, float, float, float);		//cacu betam(x1,x2,y1,y2)
float init_r(float, float, float, float);		//(X0,Y0,X1,Y1)

float cacu_um(float, float);	//(Uinf, SIGMA)
float cacu_vm(float, float);	//(Vinf, SIGMA)
...
main()
{
	...
	FILE *streamL=fopen("streamL.dat","wt");
	...
	float rmn[N]={0,}, qma=0, qmb=0, deltaL=0.02, Coeff[N]={0,}, sigmaUa=0, sigmaVa=0, sigmaUb=0, sigmaVb=0, uma=0.0, vma=0.0, umb=0.0, vmb=0.0;

	float xma=0.8, yma=0.0, xmb, ymb, xmd, ymd;

	fprintf(streamL, "%0.4f %0.4f\n", xma, yma);
//--------------------------------------------------------------------------------------------------------
//**********************************||       init Uniform flow       ||***********************************
	flow_angle=0;
	Winf = 1.0;
...
//********************************************************************************************************
	...
	//************************************||          init rmn         ||*************************************
		for(i = 0; i < N-1; i++)
		{
			rmn[i] = init_r(xma, yma, xmid[i], ymid[i]);
		}
	//********************************************************************************************************
	...
	//*********************************||           cacu the qma           ||*********************************
		for(i=0;i<N-1;i++)
		{
			qma += (Surface_gam[i] * deltaSm[i])/(2*PI*rmn[i]);
		}
	//********************************************************************************************************


	//--------------------------------------------------------------------------------------------------------
	//************************||           cacu the sigmaUa and sigmaVa           ||**************************
		for(i=0;i<N-1;i++)
		{
			sigmaUa += Surface_gam[i]*deltaSm[i]*((yma-ymid[i])/pow(rmn[i],2));
			sigmaVa += Surface_gam[i]*deltaSm[i]*((xma-xmid[i])/pow(rmn[i],2));
		}
	//********************************************************************************************************


	//--------------------------------------------------------------------------------------------------------
	//***************************||           cacu the uma and vma           ||*******************************
		uma = cacu_um(Uinf, sigmaUa);
		vma = cacu_vm(Vinf, sigmaVa);
	//********************************************************************************************************


	//--------------------------------------------------------------------------------------------------------
	//***************************||           cacu the xmb and ymb           ||*******************************
		xmb = xma + (uma/qma)*deltaL;
		ymb = yma + (vma/qma)*deltaL;
		fprintf(streamL, "%0.4f %0.4f\n", xmb, ymb);
	//********************************************************************************************************
		uma=0;vma=0;sigmaUa=0;sigmaVa=0;qma=0;
		xma = xmb; yma = ymb;
	}
...
float cacu_um(float Uinf, float SIGMA)
{
	float temp_um, temp_uinf, temp_sigma;
	temp_uinf=Uinf; temp_sigma=SIGMA;
	temp_um=temp_uinf+temp_sigma;
	return temp_um;
	temp_um=0; temp_uinf=0; temp_sigma=0;
}

float cacu_vm(float Vinf, float SIGMA)
{
	float temp_vm, temp_vinf, temp_sigma;
	temp_vinf=Vinf; temp_sigma=SIGMA;
	temp_vm=temp_vinf-temp_sigma;
	return temp_vm;
	temp_vm=0; temp_vinf=0; temp_sigma=0;
}

float init_r(float X0, float Y0, float X1, float Y1)	//(X0,Y0,X1,Y1)
{
