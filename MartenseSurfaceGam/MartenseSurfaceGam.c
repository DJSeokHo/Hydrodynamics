...
#define M 19				//number of surfaces
...
//*************************************||   Input profile and other data preparation||*************************************
	...
	for(i = 0; i < M - 1; i++)														//////////////
	{																				//   init   //
	//	slope[i] = cacu_slope(cycx[i+1], cycx[i], cycy[i+1], cycy[i]);				//  slope   //
	//	cosbetam[i] = (float)cos(slope[i]);											// cosbetam //
	//	sinbetam[i] = (float)sin(slope[i]);											// sinbetam //
...
*/
	...
	Crout_method(K_Sm_Sn_Winf,L,U,M-1);				//Matrix LU method
	ytemp[0]=rhsM_Winf[0]/L[0][0];
	for(i=1;i<M-1;i++)
	{
		ftemp=0;
		for(j=0;j<i;j++)
			ftemp+=L[i][j]*ytemp[j];
		ftemp=rhsM_Winf[i]-ftemp;
		ytemp[i]=ftemp/L[i][i];
	}
	Surface_gam_Winf[M-1-1]=ytemp[M-1-1];
	for(i=M-1-2;i>=0;i--)
	{
		ftemp=0;
		for(j=i+1;j<M-1;j++)
			ftemp+=U[i][j]*Surface_gam_Winf[j];
		Surface_gam_Winf[i]=ytemp[i]-ftemp;
	}
	for(i=0;i<M-1;i++)
	{
		printf("%d -- %0.4f\n", i+1, Surface_gam_Winf[i]);
